...
from utils.utils import accuracy, concat_all_gather, mkdir
...
class Places205(torch.utils.data.Dataset):
    def __init__(self, root, split, transform=None, target_transform=None):
        ...
        self.img_files = []
        self.labels = []
        with open(split_csv_file) as file:
            line = file.readline()
            while line:
                line = line.strip("\n")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    val_loader = torch.utils.data.DataLoader(
        valid_dataset, sampler=val_sampler, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True
    )
...
def train(train_loader, model, criterion, optimizer, epoch, args, lr_scheduler):
    ...
    progress = ProgressMeter(
        len(train_loader), [batch_time, data_time, losses, top1, top5, mAP], prefix=f"Epoch: [{epoch}]"
    )
...
def validate(val_loader, model, criterion, args):
    ...
    top5 = AverageMeter("Acc@5", ":6.2f")
    mAP = AverageMeter("mAP", ":6.2f")
    progress = ProgressMeter(len(val_loader), [batch_time, losses, top1, top5, mAP], prefix="Test: ")
    ...
    with torch.no_grad():
        ...
        print(f" * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f} mAP {mAP.avg:.3f} ")
...
def testing(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            output_list, _max_index = torch.max(output_list, dim=0)
            output = output_list
            images = images[0]
            acc1, acc5 = accuracy(output, target, topk=(1, 5))
            acc1 = torch.mean(concat_all_gather(acc1.unsqueeze(0)), dim=0, keepdim=True)
        ...
        print(f"$$our final calculated accuracy {final_accu:.7f}")
    return top1.avg
...
def sanity_check(state_dict, pretrained_weights):
    """Linear classifier should not change any weights other than the linear layer. This sanity check asserts nothing
    wrong happens (e.g., BN stats updated).
    """
    ...
    for k in list(state_dict.keys()):
        ...
        assert (state_dict[k].cpu() == state_dict_pre[k_pre]).all(), f"{k} is changed in linear classifier training."
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if args.cos:
        lr = 0.5 * (1.0 + math.cos(math.pi * epoch / args.epochs)) * (lr - end_lr) + end_lr
    else:
