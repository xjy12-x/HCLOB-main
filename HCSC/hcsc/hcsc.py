...
class HCSC(nn.Module):
    """Our proposed HCSC framework with instance selection and prototype selection.
    ...
    def forward(self, images, is_eval=False, cluster_result=None, index=None):
        """Input: images: a list of images, where images[0] as im_q and images[1] as im_k others are local views, which
        are also treated as keys is_eval: return momentum embeddings (used for clustering) cluster_result:
        cluster assignments, centroids, and density index: indices for training samples Output: logits, targets,
        ...
        _proto_logits, proto_labels, proto_selected, temp_protos = self.get_protos(q, index, cluster_result)
        # 正样本相似度计算
        l_pos = torch.einsum("nc,nc->n", [q, k]).unsqueeze(-1)
        # negative logits: Nxr
        if proto_labels is not None and self.instance_selection:
            try:
                self.selected_masks, sample_ratios = self.sample_neg_instance(
                    cluster_result["im2cluster"], proto_selected, temp_protos, index
                )
        ...
        # dequeue and enqueue
        _local_proto_logits, _local_proto_targets = None, None
        _local_logits, _local_labels = None, None
        # computing local logits when enabling multi-crop
        if self.multi_crop:
            _local_logits, _local_labels = self.compute_local_logits(q, k, local_views, index)
            _local_proto_logits, _local_proto_targets = self.compute_local_proto_logits(
        ...
        loss = nn.CrossEntropyLoss()(logits, labels)  # 最大化正样本相似度，最小化负样本相似度
    ...
    def compute_local_proto_logits(self, local_views, proto_selected, cluster_result, index):
        ...
        if cluster_result is not None:
            ...
            for local_view in local_views:
                ...
                proto_logits, proto_labels, _proto_selected, _temp_protos = self.get_protos(
                    local_view, index, cluster_result
                )
    ...
    def compute_local_logits(self, q, k, local_views, index):
        ...
        Args:
            q: (torch.Tensor([N, D]))
            k: (torch.Tensor([N, D]))
            local_views: (list[torch.Tensor([N, D])]) features of local views that could be additional keys or queries.
        ...
        if self.selection_on_local and hasattr(self, "selected_masks"):
            ...
            for selected_mask in self.selected_masks:
                l_pos_list.extend(
                    [torch.einsum("nc,nc->n", [local_view, k]).unsqueeze(-1) for local_view in local_views]
                )
                for local_view in local_views:
            ...
            l_neg_list = [
                torch.einsum("nc,ck->nk", [local_view, self.queue.clone().detach()]) for local_view in local_views
            ]
            ...
            for n, (im2cluster, prototypes, density) in enumerate(
                ...
                if self.proto_selection:
                    if n == (len(cluster_result["im2cluster"]) - 1):
                        ...
                        neg_proto_id = torch.LongTensor(neg_proto_id).to(pos_proto_id.device)
                        neg_prototypes = prototypes[neg_proto_id]  # [N_neg, D]
                        neg_mask = self.sample_neg_protos(
                            im2cluster, cluster2cluster, pos_proto_id, prot_logits, n, cluster_result
                        )  # [N, N_neg]
