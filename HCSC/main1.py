...
parser.add_argument(
    "--cluster-nums", default=[3000, 2000, 1000], nargs="*", type=int, help="number of clusters at each level"
)
# 在参数解析部分添加
parser.add_argument("--warmup-epoch", default=10, type=int, help="number of warm-up epochs before clustering")
# 模型核心参数
...
class HCSC(nn.Module):
    ...
    def forward(self, images, is_eval=False, cluster_result=None, index=None):
        ...
        cluster assignments, centroids, and density index: indices for training samples Output: total_loss: 总损失.
        """
        if is_eval:
...
def run_hkmeans(x, args):
    """This function is a hierarchical k-means 修改：添加GPU检测和回退机制.
    """
    ...
    try:
        ...
        if hasattr(faiss, "StandardGpuResources"):
            ...
            print("⚠️  GPU版本faiss不可用，将使用CPU版本")
    except:
        print("⚠️  无法检测GPU支持，将使用CPU版本")
...
def hcsc_train(model, train_loader, optimizer, epoch, args, cluster_result=None):
    ...
    for batch_idx, (images, index) in enumerate(train_bar):
        ...
        train_bar.set_postfix(
            {"Loss": f"{loss_value:.4f}", "AvgLoss": f"{avg_loss:.4f}", "Batch": f"{batch_idx + 1}/{len(train_loader)}"}
        )
        ...
        if epoch >= args.warmup_epoch:
            ...
            if all_features:
                ...
                try:
                    ...
                    print(
                        f"✅ 聚类成功: 层级数={len(cluster_result['im2cluster'])}, "
