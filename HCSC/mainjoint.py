...
from hcsc.loader import GaussianBlur, Solarize, TwoCropsTransform_JointCrop
...
parser.add_argument(
    "--local-crop-scale", default=[0.05, 0.4], nargs="*", type=float, help="scale range for local crops"
)
...
train_transform = TwoCropsTransform_JointCrop(
    transforms.Compose(augmentation1), transforms.Compose(augmentation2), scale=(args.crop_min, 1.0)
)
train_data = CIFAR10(
        ...
        try:
            ...
            if args.multi_crop and len(batch) > 2:
                ...
                for view in batch[2:]:
                    ...
                    assert view_tensor.dim() == 4, "local view 必须是 4D 张量"
