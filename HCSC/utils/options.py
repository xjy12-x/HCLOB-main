...
def parse_args_main():
    ...
    parser = argparse.ArgumentParser(description="HCSC Training")

    # 添加debug参数
    parser.add_argument("--debug", action="store_true", help="Enable debug mode")
    parser.add_argument("--data", default="./data/", type=str, help="path to dataset directory")
    parser.add_argument(
        "--dataset",
        type=str,
        default="imagenet",
        choices=["imagenet", "cifar10", "cifar100", "coco"],
    ...
    parser.add_argument(
        "--schedule", default=[120, 160], nargs="*", type=int, help="learning rate schedule (when to drop lr by 10x)"
    )
    ...
    parser.add_argument(
        "--dist-url", default="tcp://localhost:10034", type=str, help="url used to set up distributed training"
    )
    ...
    parser.add_argument("--aug-plus", type=int, default=1, help="use moco-v2/SimCLR data augmentation")
    parser.add_argument("--cos", type=int, default=1, help="use cosine lr schedule")
    parser.add_argument("--num-cluster", default="3000,2000,1000", type=str, help="number of clusters")
    parser.add_argument(
        "--warmup-epoch", default=20, type=int, help="number of warm-up epochs to only train with InfoNCE loss"
    )

    parser.add_argument(
        "--multi_crop", action="store_true", default=False, help="Whether to enable multi-crop transformation"
    )
    parser.add_argument("--nmb_crops", type=int, default=[1, 1, 1, 1, 1], nargs="+", help="list of number of crops")
    parser.add_argument("--size_crops", type=int, default=[224, 192, 160, 128, 96], nargs="+", help="crops resolutions")
    parser.add_argument(
        "--min_scale_crops",
        type=float,
        default=[0.2, 0.172, 0.143, 0.114, 0.086],
        nargs="+",
    ...
    parser.add_argument(
        "--max_scale_crops",
        type=float,
        default=[1.0, 0.86, 0.715, 0.571, 0.429],
        nargs="+",
        help="argument in RandomResizedCrop",
    )
    ...
    parser.add_argument("--exp-dir", default="experiment_pcl", type=str, help="experiment directory")
...
def parse_args_lincls_imagenet():
    ...
    parser.add_argument(
        "--lr", "--learning-rate", default=5.0, type=float, metavar="LR", help="initial learning rate", dest="lr"
    )
    parser.add_argument(
        "--schedule", default=[60, 80], nargs="*", type=int, help="learning rate schedule (when to drop lr by a ratio)"
    )
    ...
    parser.add_argument(
        "--dist-url", default="tcp://localhost:10028", type=str, help="url used to set up distributed training"
    )
...
def parse_args_lincls_places():
    ...
    parser.add_argument(
        "--lr", "--learning-rate", default=3.0, type=float, metavar="LR", help="initial learning rate", dest="lr"
    )
    parser.add_argument(
        "--schedule", default=[60, 80], nargs="*", type=int, help="learning rate schedule (when to drop lr by a ratio)"
    )  # default is for places205
    parser.add_argument("--cos", type=int, default=1, help="use cosine lr schedule")
...
def parse_semisup_args():
    ...
    parser.add_argument(
        "--labels_perc", type=str, default="10", choices=["1", "10"], help="fine-tune on either 1% or 10% of labels"
    )
    parser.add_argument("--exp_dir", type=str, default=".", help="experiment dump path for checkpoints and log")
    parser.add_argument("--seed", type=int, default=31, help="seed")
    parser.add_argument("data_path", type=str, default="", help="path to imagenet")
    ...
    parser.add_argument(
        "--batch_size", default=32, type=int, help="batch size per gpu, i.e. how many unique instances per gpu"
    )
    ...
    parser.add_argument(
        "--decay_epochs", type=int, nargs="+", default=[30, 60], help="Epochs at which to decay learning rate."
    )
...
def parse_args_knn():
    ...
    parser.add_argument("--use_cuda", default=1, type=int, help="Store features in GPU.")
    parser.add_argument("--arch", default="resnet50", type=str, help="Architecture")
    parser.add_argument("--checkpoint_key", default="state_dict", type=str, help="Key to use in the checkpoint")
    parser.add_argument(
        "--dump_features", default=None, help="Path where to save computed features, empty for no saving"
    )
    parser.add_argument(
        "--load_features",
        default=None,
