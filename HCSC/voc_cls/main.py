...
def main(args, data_dir, model_name, num, lr, epochs, batch_size=16, download_data=False):
    ...
    Args:
        ...
        num: model_num for file management purposes (can be any positive integer. Your results stored will have this
            number as suffix)
    ...
    transformations = transforms.Compose(
        [
            ...
            transforms.RandomChoice(
                [transforms.ColorJitter(brightness=(0.80, 1.20)), transforms.RandomGrayscale(p=0.25)]
            ),
    ...
    log_file = open(os.path.join(args.log, f"log-{num}.txt"), "w+")
    model_dir = os.path.join(args.log, "model")
    if not os.path.exists(model_dir):
        os.mkdir(model_dir)
    log_file.write(f"----------Experiment {num} - {model_name}-----------\n")
    log_file.write(f"transformations == {transformations.__str__()}\n")
    trn_hist, val_hist = train_model(
        model, device, optimizer, scheduler, train_loader, valid_loader, model_dir, num, epochs, log_file
    )
    torch.cuda.empty_cache()

    plot_history(trn_hist[0], val_hist[0], "Loss", os.path.join(model_dir, f"loss-{num}"))
    plot_history(trn_hist[1], val_hist[1], "Accuracy", os.path.join(model_dir, f"accuracy-{num}"))
    log_file.close()
    ...
    weights_file_path = os.path.join(model_dir, f"model-{num}.pth")
    ...
    gt_path, scores_path, scores_with_gt_path = (
        os.path.join(model_dir, f"gt-{num}.csv"),
        os.path.join(model_dir, f"scores-{num}.csv"),
        os.path.join(model_dir, f"scores_wth_gt-{num}.csv"),
    )
    ...
    utils.get_classification_accuracy(gt_path, scores_path, os.path.join(model_dir, f"clf_vs_threshold-{num}.png"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-j", "--workers", default=4, type=int, metavar="N", help="number of data loading workers (default: 32)"
    )
    ...
    parser.add_argument(
        "--lr", "--learning-rate", default=0.05, type=float, metavar="LR", help="initial learning rate", dest="lr"
    )
