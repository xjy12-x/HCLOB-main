            ...
            if phase == "train":
                ...
                for data, target in tqdm(train_loader):
                    ...
                    running_ap += get_ap_score(
                        torch.Tensor.cpu(target).detach().numpy(), torch.Tensor.cpu(m(output)).detach().numpy()
                    )
...
def test(model, device, test_loader, returnAllScores=False):
    ...
    Args:
        ...
        returnAllScores: If true additionally return all confidence scores and ground truth
    ...
    print(f"test_loss: {avg_test_loss:.4f}, test_avg_precision:{test_map:.3f}")

    if not returnAllScores:
