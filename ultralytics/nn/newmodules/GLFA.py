...
class QKV_block(nn.Module):  # 定义一个QKV_block类，继承自nn.Module
    ...
    def forward(self, x):  # 前向传播函数，输入x是一个4D张量(batch_size, channels, height, width)
        b, c, h, w = x.size()  # 获取输入x的尺寸，b为批量大小，c为通道数，h和w分别为高度和宽度
...
class GLFA(nn.Module):  # 定义GLFA类，继承自nn.Module
    def __init__(self, in_channels):  # 初始化函数，接受输入通道数
        ...
        self.W = nn.Conv2d(in_channels=self.in_channels, out_channels=self.out_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):  # 前向传播函数，输入x是一个4D张量(batch_size, channels, height, width)
        ...
        # 捕捉全局特征
        batch_size, h, w = x.size(0), x.size(2), x.size(3)  # 获取输入的尺寸
        ...
        sim_map = (self.in_channels ** -.5) * sim_map  # 缩放相似度矩阵，避免数值不稳定
...
class C3k2_GLFA(C2f_GLFA):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else GLFA(self.c) for _ in range(n)
        )
