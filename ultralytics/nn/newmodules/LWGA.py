...
class LWGA(nn.Module):
    def __init__(self, c1, c2=None, stage=1, att_kernel=7, mlp_ratio=4.0, 
                 drop_path=0.1, act_layer=None, norm_layer=None, *args, **kwargs):
        """
        LWGA模块 - 适配YOLO框架
        参数:
            c1: 输入通道数 (由YOLO自动传递)
            c2: 输出通道数，如果为None则使用c1
            stage: 阶段，默认1
            att_kernel: 注意力核大小，默认7
            mlp_ratio: MLP扩展比例，默认4.0
            drop_path: DropPath比率，默认0.1
        ...
        print(f"LWGA配置: 输入={c1}, 输出={c2}, stage={stage}, att_kernel={att_kernel}, "
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        shortcut = x.clone()
        x1, x2, x3, x4 = torch.split(x, [self.dim_split, self.dim_split, 
                                         self.dim_split, self.dim_split], dim=1)
