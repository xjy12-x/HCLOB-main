...
class MSEA(nn.Module):  # MSEA多尺度特征增强聚合模块
    def __init__(self, dim, ratio=16):  # 初始化MSEA模块，dim是输入通道数，ratio用于通道注意力的压缩比
        ...
        #多尺度卷积分支
        self.branch_conv0 = nn.Conv2d(in_channels=self.dim_1, out_channels=self.dim_1, kernel_size=1)  # 1x1卷积，不改变通道数
        self.branch_conv1 = nn.Conv2d(in_channels=4*self.dim_1 // 8,
                                      ...
                                      dilation=3, padding=(1+6*3) // 2)  # 7x7卷积，扩张率3，适合提取较大尺度的特征
        self.branch_conv2 = nn.Conv2d(in_channels=3*self.dim_1 // 8,
                                      ...
                                      dilation=2, padding=(1+4*2) // 2)  # 5x5卷积，扩张率2，适合提取中等尺度特征
        self.branch_conv3 = nn.Conv2d(in_channels=self.dim_1 // 8,
                                      out_channels=self.dim_1 // 8,
                                      stride=1, kernel_size=3, groups=self.dim_1 // 8,
                                      dilation=1, padding=(1+2*1) // 2)  # 3x3卷积，扩张率1，适合提取细节级别的特征
        self.P_conv = nn.Conv2d(in_channels=self.dim_1, out_channels=self.dim_1, kernel_size=1)  # 1x1卷积，用于最终输出的特征整合
        # 深度卷积分支
        self.avg_pool_1 = nn.AvgPool2d(7, 1, 3)  # 7x7平均池化，步长为1，padding为3，用于降维和捕捉全局信息

        self.conv1X1 = nn.Sequential(nn.Conv2d(self.dim_1, self.dim_1, 1),  # 1x1卷积，保持通道数不变
                                     nn.BatchNorm2d(self.dim_1),  # 批归一化
                                     nn.ReLU(inplace=True))  # ReLU激活函数

        self.depth_convs = nn.Sequential(
            nn.Conv2d(self.dim_1, self.dim_1, kernel_size=(1, 7), padding=(0, 7 // 2), groups=self.dim_1),  # 1x7深度卷积
            nn.Conv2d(self.dim_1, self.dim_1, kernel_size=(7, 1), padding=(7 // 2, 0), groups=self.dim_1),  # 7x1深度卷积
            nn.Conv2d(self.dim_1, self.dim_1, kernel_size=(1, 11), padding=(0, 11 // 2), groups=self.dim_1),  # 1x11深度卷积
            nn.Conv2d(self.dim_1, self.dim_1, kernel_size=(11, 1), padding=(11 // 2, 0), groups=self.dim_1)  # 11x1深度卷积
        )
        # 计算通道注意力
    ...
    def forward(self, x):
        ...
        x1_1, x1_2, x1_3 = torch.split(x1, [4*self.dim_1 // 8, 3*self.dim_1 // 8, self.dim_1 // 8], dim=1)  # 分割x1为三个部分

        x1_1 = self.branch_conv1(x1_1)  # 对x1_1应用7x7卷积
        x1_2 = self.branch_conv2(x1_2)  # 对x1_2应用5x5卷积
        x1_3 = self.branch_conv3(x1_3)  # 对x1_3应用3x3卷积

        x1_out = self.act1(self.P_conv(torch.cat([x1_1, x1_2, x1_3], dim=1)))  # 将三个特征拼接并应用1x1卷积，最后通过ReLU激活

        x2_out = self.act2(self.conv1X1(self.depth_convs(self.conv1X1(self.avg_pool_1(x2)))))  # 通过深度卷积提取x2的特征，并应用ReLU和Sigmoid激活
...
class C3k2_MSEA(C2f_MSEA):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else MSEA(self.c) for _ in range(n)
        )
