...
class ChannelAttention(nn.Module):  # 定义 ChannelAttention 类，实现通道级别的注意力机制
    def __init__(self, input_channels, internal_neurons):  # 初始化 ChannelAttention 类，input_channels 为输入通道数，internal_neurons 为中间神经元数
        super(ChannelAttention, self).__init__()
        self.fc1 = nn.Conv2d(in_channels=input_channels, out_channels=internal_neurons, kernel_size=1, stride=1, bias=True)  # 1x1 卷积，降低通道数到 internal_neurons
        self.fc2 = nn.Conv2d(in_channels=internal_neurons, out_channels=input_channels, kernel_size=1, stride=1, bias=True)  # 1x1 卷积，恢复通道数到 input_channels
        self.input_channels = input_channels  # 保存输入的通道数，用于调整输出形状

    def forward(self, inputs):  # 定义前向传播方法
        x1 = F.adaptive_avg_pool2d(inputs, output_size=(1, 1))  # 对输入进行自适应平均池化，输出为 (1, 1) 尺寸，压缩空间维度
...
class SpatialAttention(nn.Module):  # 定义 SpatialAttention 类，实现空间级别的注意力机制
    def __init__(self, in_channels):  # 初始化 SpatialAttention 类，in_channels 为输入的通道数
        super().__init__()
        self.dconv5_5 = nn.Conv2d(in_channels, in_channels, kernel_size=5, padding=2, groups=in_channels)  # 使用 5x5 深度可分离卷积
        self.dconv1_7 = nn.Conv2d(in_channels, in_channels, kernel_size=(1, 7), padding=(0, 3), groups=in_channels)  # 使用 1x7 深度可分离卷积
        self.dconv7_1 = nn.Conv2d(in_channels, in_channels, kernel_size=(7, 1), padding=(3, 0), groups=in_channels)  # 使用 7x1 深度可分离卷积
        self.dconv1_11 = nn.Conv2d(in_channels, in_channels, kernel_size=(1, 11), padding=(0, 5), groups=in_channels)  # 使用 1x11 深度可分离卷积
        self.dconv11_1 = nn.Conv2d(in_channels, in_channels, kernel_size=(11, 1), padding=(5, 0), groups=in_channels)  # 使用 11x1 深度可分离卷积
        self.dconv1_21 = nn.Conv2d(in_channels, in_channels, kernel_size=(1, 21), padding=(0, 10), groups=in_channels)  # 使用 1x21 深度可分离卷积
        self.dconv21_1 = nn.Conv2d(in_channels, in_channels, kernel_size=(21, 1), padding=(10, 0), groups=in_channels)  # 使用 21x1 深度可分离卷积
...
class SCSPA(nn.Module):  # SCSPA 协同通道空间像素注意力模块
    def __init__(self, in_channels, ratio=4):  # 初始化 SCSPA 类，in_channels 为输入通道数，ratio 为通道注意力中间神经元的比例
        super(SCSPA, self).__init__()
        self.ca = ChannelAttention(input_channels=in_channels, internal_neurons=in_channels // ratio)  # 初始化通道注意力模块
...
class C2f_SCSPA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(Bottleneck_SCSPA(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class C3k2_SCSPA(C2f_SCSPA):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_SCSPA(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n)
        )
